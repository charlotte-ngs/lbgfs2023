...
In livestock breeding we are mainly concerned with changing animal populations at the genetic level. The reason why we are interested in changing a population genetically is because parents do not pass their phenotypes to their offspring. Parents pass a random sample of their genes or alleles to their offspring. For each offspring every parent does transmit a different sample of their genes. For answering the fundamental questions of livestock breeding which aim to find the best parents for future generations, we would have to know which animals would pass the most valuable genes or alleles to potential offspring when selecting these animals as parents. The value of genes or alleles with respect to a certain trait of interest cannot be measured or observed directly. Hence, we have to estimate those values. This can be done by using the effect that phenotypes measured or observed in an animal always have a genetic background. The genetic background for a given phenotype can be demonstated using the central dogma of molecular biology (https://en.wikipedia.org/wiki/Central_dogma_of_molecular_biology). This dogma describes the relationship between phenotypes ($Y$) and genotypes ($V$) by the following equation \@ref(eq:phengenenv)
